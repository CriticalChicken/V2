<ul id="bold-menu-items">
<li><a href="<?php echo get_site_url(); ?>/latest">Latest</a></li>
<li><a href="<?php echo get_site_url(); ?>/section/news">News</a></li>
<li><a href="<?php echo get_site_url(); ?>/section/features">Features</a></li>
<li><a href="<?php echo get_site_url(); ?>/section/reviews">Reviews</a></li>
<li><a href="<?php echo get_site_url(); ?>/explore">Explore</a></li>
</ul><!-- /bold-menu-items -->
<ul id="regular-menu-items">
<li><a href="<?php echo get_site_url(); ?>/about">About</a></li>
<li><a href="<?php echo get_site_url(); ?>/webring">Webring</a></li>
<li><a href="mailto:office@criticalchicken.com">Contact</a></li>
</ul><!-- /regular-menu-items -->
